...
describe('convertFromBaseNToDecimal()', () => {
  describe('When given a number in a base that is less than or equal to 10', () => {
    test('should convert it to decimal', () => {
      ...
      expect(converter.convertFromBaseNToDecimal('10010', 2)).toEqual(18)
    })
  })
  describe('When given a number in a base that is greater then 10', () => {
    test('should convert it to decimal', () => {
  ...
  describe('When given an invalid number in base N', () => {
    test('should throw an error indicating that the number is invalid', () => {
      expect(() => {
        converter.convertFromBaseNToDecimal('1X1', 11)
      }).toThrowError('Invalid number')
      expect(() => {
        converter.convertFromBaseNToDecimal('123', 3)
      }).toThrowError('Invalid number')
      expect(() => {
        converter.convertFromBaseNToDecimal('12@', 3)
      }).toThrowError('Invalid number')
    })
    ...
    test('should throw an error indicating that the number is not a string', () => {
      expect(() => converter.convertFromBaseNToDecimal(1010 as unknown as string, 11)).toThrowError(
